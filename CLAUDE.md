...
- **Vence a la medianoche (Bogotá) al terminar el día marcado** (23-sep-2026; antes era a las 12:00 m): una tarea para hoy se puede entregar hasta las 11:59 pm. Todo cálculo de fecha límite pasa por `lib/utils/due-date.ts` (`parseDueDate`, `dueDeadline`, `isPastDue`, `formatDueDate`, `dueDateKey`); nunca `new Date(task.dueDate)` suelto. Motivo: `new Date('2026-09-23')` es medianoche UTC = el 22 a las 7 pm en Bogotá, y las tareas salían vencidas (y mostradas) un día antes. El día de la tarea es la fecha UTC de lo guardado, así que las filas viejas (00:00Z) y las nuevas (17:00Z) funcionan igual sin migrar datos. Lo guardado es un **ancla** al mediodía (`dueDayAnchor`), no el vencimiento: guardar la medianoche (05:00Z del día siguiente) correría el día.
